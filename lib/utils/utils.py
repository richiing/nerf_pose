...
def load_nerf_panoptic(model, pretrained_file):
    this_dir = os.path.dirname(__file__)
    pretrained_file = os.path.abspath(os.path.join(this_dir, '../..', pretrained_file))
    pretrained_state_dict = torch.load(pretrained_file)
    model.module.model.load_state_dict(pretrained_state_dict)
    print("load nerf statedict from {}".format(pretrained_file))
    return model