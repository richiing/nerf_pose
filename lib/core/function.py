...
def train_3d(config, model, optimizer, loader, epoch, output_dir, writer_dict, device=torch.device('cuda'), dtype=torch.float):
    ...
    for i, (inputs, targets_2d, weights_2d, targets_3d, meta, input_heatmap, ray_batches) in enumerate(loader):
        data_time.update(time.time() - end)

        if 'panoptic' in config.DATASET.TEST_DATASET:
            pred, heatmaps, grid_centers, loss_2d, loss_3d, loss_cord, loss_nerf = model(views=inputs, meta=meta,
            ...
            pred, heatmaps, grid_centers, loss_2d, loss_3d, loss_cord, loss_nerf = model(meta=meta, targets_3d=targets_3d[0],
        ...
        loss = loss_2d + loss_3d + loss_cord + loss_nerf
        losses.update(loss.item())

        if loss_cord > 0:
            optimizer.zero_grad()
            (loss_2d + loss_cord + loss_nerf).backward()
            optimizer.step()
        ...
        if i % config.PRINT_FREQ == 0:
            ...
            msg = 'Epoch: [{0}][{1}/{2}]\t' \
                  ...
                  'Memory {memory:.1f}'.format(
                    ...
                    loss_cord=losses_cord, loss_nerf=losses_nerf, memory=gpu_memory_usage)
            logger.info(msg)
...
def validate_3d(config, model, loader, output_dir):
    ...
    with torch.no_grad():
        ...
        for i, (inputs, targets_2d, weights_2d, targets_3d, meta, input_heatmap, ray_batches) in enumerate(loader):
            data_time.update(time.time() - end)
            if 'panoptic' in config.DATASET.TEST_DATASET:
                pred, heatmaps, grid_centers = model(views=inputs, meta=meta, targets_2d=targets_2d,
                                                              weights_2d=weights_2d, targets_3d=targets_3d[0])
            elif 'campus' in config.DATASET.TEST_DATASET or 'shelf' in config.DATASET.TEST_DATASET:
                pred, heatmaps, grid_centers = model(meta=meta, targets_3d=targets_3d[0],
                                                              input_heatmaps=input_heatmap)
