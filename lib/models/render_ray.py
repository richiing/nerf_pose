...
def _compute_projection(img_meta):
    ...
    index_list = []
    for cam in cameras:
        index_list.append(dict((k, v[index]) for k, v in cam.items()))
    return index_list
...
def render_rays_func(ray_o,
    ...
    if mode == "image":
        img = img.unsqueeze(0)# [1, nview, h, w, rgb]
        train_camera = _compute_projection(img_meta)
        # views = nerf_sample_view
        # mv_rgb = []
        # mv_den = []
        # if is_train:
        ...
        rgb_feat, mask = projector.compute(pts, img, train_camera, features_2D, grid_sample=True, resize_transform=resize_transform)
        # RGB_feat: [N_rays, N_samples, N_views, channel], mask: [n_rays, n_samples, n_views, 1]
...
def render_rays(ray_batch=None,
                mean_volume=None,
                cov_volume=None,
                features_2D=None,
                img=None,
    ...
    gt_depth = None
    nerf_sizes = None
    if 'gt_depth' in ray_batch:
        gt_depth = ray_batch['gt_depth']
    if 'nerf_sizes' in ray_batch:
